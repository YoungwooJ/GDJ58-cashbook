...
<%
	...
	ArrayList<HashMap<String, Object>> list = statsDao.selectCashStatsByMonth(loginMember.getMemberId(), year);
						...
						<div class="col-12 col-lg-8 col-xxl-9 d-flex order-3 order-xxl-2">
							<div class="card flex-fill">
								<div class="card-header">
									...
									<h5 class="card-title mb-0"><%=year%>년 월별 가계부 통계</h5>
								</div>
								<div class="card-body d-flex">
									<div class="align-self-center w-100">
										<div class="chart">
											...
											<table class="table">
												...
												%>
														<tr>
															<td>
																<a class="badge bg-primary" href="<%=request.getContextPath()%>/stats/statsListByDay.jsp?year=<%=year%>&month=<%=m.get("month")%>">
																	<%=m.get("month")%>월
																</a>
															</td>
											...
											<%
												if(year > minYear){
											%>
													<a class="btn btn-primary" href="<%=request.getContextPath()%>/stats/statsListByMonth.jsp?year=<%=year-1%>">이전년</a>
											<%		
												}
											
												if(year < maxYear){
											%>
													<a class="btn btn-primary" href="<%=request.getContextPath()%>/stats/statsListByMonth.jsp?year=<%=year+1%>">다음년</a>
											<%		
												}
											%>
										</div>
